...
function generateReading(card, position) {
  ...
  const keywords = card.isReversed ? card.keywords_r : card.keywords_u;
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
      ...
      {flipped && (
        <div style={{
          ...
          <div style={{ fontSize: "11px", color: card.isReversed ? "#d4856a" : "#8cc87e", letterSpacing: "0.1em", marginBottom: "4px" }}>
...
export default function TarotReader() {
  ...
  // Load all saved readings on mount
  useEffect(() => {
    try {
      const saved = localStorage.getItem('tarotReadings');
      if (saved) setSavedReadings(JSON.parse(saved));
    } catch (e) {
      console.error('Failed to load saved readings', e);
    }
  }, []);

  // Save reading whenever it changes
  useEffect(() => {
    if (cards.length > 0 && readings.length > 0 && allRevealed) {
      const readingData = {
        spread,
        cards,
        readings,
      ...
      try {
        const updatedReadings = [readingData, ...savedReadings];
  ...
  }, [cards, readings, allRevealed, spread, question]);
  ...
  function viewSavedReading(index) {
    if (savedReadings[index]) {
        ...
        {phase === "home" && (
          <div style={{ animation: "fadeUp 0.6s ease" }}>
            ...
            {savedReadings.length > 0 && (
              <div style={{ ...thinBorder, borderRadius: "12px", padding: "24px", marginTop: "24px", background: "rgba(200,169,110,0.03)" }}>
                <div style={{ ...goldText, fontSize: "11px", letterSpacing: "0.2em", marginBottom: "16px", textAlign: "center" }} className="cinzel">
                  READING HISTORY ({savedReadings.length})
                </div>

                {!showHistory ? (
                  <>
                    <div style={{ ...thinBorder, borderRadius: "8px", padding: "16px", marginBottom: "12px", background: "rgba(200,169,110,0.02)" }}>
                      <div style={{ fontSize: "12px", ...goldText, marginBottom: "6px" }} className="cinzel">
                        MOST RECENT
                      </div>
                      <div style={{ fontSize: "14px", fontWeight: 500, marginBottom: "4px" }} className="cinzel">
                        {SPREADS[savedReadings[0].spread].label}
                      </div>
                      {savedReadings[0].question && (
                        <div style={{ ...mutedText, fontSize: "11px", fontStyle: "italic", marginBottom: "4px" }} className="cormorant">
                          "{savedReadings[0].question}"
                        </div>
                      )}
                      <div style={{ ...mutedText, fontSize: "10px", letterSpacing: "0.05em" }} className="cinzel">
                        {new Date(savedReadings[0].timestamp).toLocaleString()}
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button 
                        ...
                        onClick={() => viewSavedReading(0)}
                      >
                        <span style={{ ...goldText, letterSpacing: "0.1em" }} className="cinzel">
                          VIEW READING
                        </span>
                      </button>
                  ...
                  <>
                    <div style={{ maxHeight: "300px", overflowY: "auto", marginBottom: "12px" }}>
                      {savedReadings.map((reading, idx) => (
                        <button
                          key={idx}
                          className="spread-btn"
                          style={{ marginBottom: "8px", padding: "12px", textAlign: "left", background: selectedHistoryIndex === idx ? "rgba(200,169,110,0.15)" : "transparent" }}
                          onClick={() => setSelectedHistoryIndex(idx)}
                        >
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: "12px", fontWeight: 500, marginBottom: "2px" }} className="cinzel">
                                {SPREADS[reading.spread].label}
                              </div>
                              {reading.question && (
                                <div style={{ ...mutedText, fontSize: "11px", fontStyle: "italic" }} className="cormorant">
                                  "{reading.question.substring(0, 35)}{reading.question.length > 35 ? '...' : ''}"
                                </div>
                              )}
                            </div>
                            <div style={{ ...mutedText, fontSize: "9px", whiteSpace: "nowrap", marginLeft: "8px" }} className="cinzel">
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button 
                        className="spread-btn" 
                        style={{ flex: 1, padding: "10px 12px", fontSize: "12px" }}
                        onClick={() => selectedHistoryIndex !== null && viewSavedReading(selectedHistoryIndex)}
                        disabled={selectedHistoryIndex === null}
                      >
                        <span style={{ ...goldText, letterSpacing: "0.1em", opacity: selectedHistoryIndex === null ? 0.5 : 1 }} className="cinzel">
                          VIEW SELECTED
                        </span>
                      </button>
                      <button 
                        className="spread-btn" 
                        style={{ flex: 1, padding: "10px 12px", fontSize: "12px" }}
                        onClick={() => { setShowHistory(false); setSelectedHistoryIndex(null); }}
                      >
                        <span style={{ ...goldText, letterSpacing: "0.1em" }} className="cinzel">
                          BACK
                        </span>
                ...
                <div style={{ display: "flex", gap: "16px", alignItems: "flex-start", marginBottom: "16px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ ...mutedText, fontSize: "12px", letterSpacing: "0.1em" }} className="cormorant">
                      {cards[activeCard].isReversed ? "↓ Reversed" : "↑ Upright"}
                    ...
                    <div style={{ ...mutedText, fontSize: "12px", marginTop: "6px", fontStyle: "italic" }} className="cormorant">
                      {(cards[activeCard].isReversed ? cards[activeCard].keywords_r : cards[activeCard].keywords_u).join(" · ")}
                    </div>
                  </div>
                </div>
